...
func (r *RedisRepo) FindByID(ctx context.Context, id uint64) (model.Order, error) {
	...
	if errors.Is(err, redis.Nil) { // check if error returned is a redis.Nil error, if yes means value cannot be found from the key
	...
	err = json.Unmarshal([]byte(res), &order) //	will write the byte array or string back into a json, which according to the struct tags, will reconstuct the order object
	if err != nil {
...
func (r *RedisRepo) Update(ctx context.Context, order model.Order) error { // can fail incase of failed communication or duplicate ID, so need to return error
	...
	*
	...
	key := orderIDKey(order.OrderID) // get the redis key as string
	//res := r.Client.Set(ctx, key, string(data), 0)	// cast the byte array into a string, but Set will override data that alr exists w the same key
	err = r.Client.SetXX(ctx, key, string(data), 0).Err() // cast the byte array into a string, only set value when it alr exists
